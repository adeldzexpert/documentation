...
employee's approver for the *Attendances* application, that user will be able to view the attendance
records for that specific employee, as well as make modifications to that employee's attendance
records (when necessary). This applies to all employee's the user is listed as the *Attendances*
...
- :guilabel:`Attendance Kiosk Url`: Odoo generates a unique web address (url) in order to use a
  ...
  .. important::
     ...
     access the Attendances application kiosk. If necessary, such as a security breach, click
     :guilabel:`Generate a new Kiosk Mode URL` to generate a new url, located beneath the link, and
     update the kiosk(s) accordingly.
...
To check in or out using a badge, tap the :guilabel:`📷 Tap to scan` image in the center of the
kiosk. Then, scan the barcode on the badge using the method configured in the :ref:`kiosk settings
...
above the numbers. Tap in the PIN number using the number pad, then tap :guilabel:`OK` when done.
The employee is then checked in or out, and a :ref:`confirmation message
<attendances/confirmation>` appears.

.. image:: attendances/enter-pin.png
   :align: center
   :alt: The pop-up that appears when prompted to enter a pin number.
...
drop-down button in the :guilabel:`Search` bar above the dashboard, then select a filter or group.
There are several pre-configured filters and groups to chose from, as well as an option to create
custom ones.
...
To default view is a :guilabel:`Graph`. To view the data in a pivot table, click the
